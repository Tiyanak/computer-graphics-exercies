		...
		void calculateSplajn(int newTime = 500) {
			...
			for (int i = 0; i < bSplajnSize - 3; i++) {
				...
				for (float t = 0.0f; t <= 1.0f; t += time) {
					...
					// pomakni se iy tocke u smjeru tangente
					tockeKrivulje[tangenteCounter] = Vertex::Vertex(
								tockaKrivulje[0] + tangente[tangenteCounter].x,
								tockaKrivulje[1] + tangente[tangenteCounter].y,
								tockaKrivulje[2] + tangente[tangenteCounter].z);
...
Obj object = Obj::Obj(string("objekti/Neghvar.obj"));
...
void myDisplay(void)
{
	...
	// napravi translaciju necega
	Vertex tockaKrivulje = spline.tockeKrivulje[iteration];
	glTranslatef(tockaKrivulje.x, tockaKrivulje.y, tockaKrivulje.z);

	// izracunaj rotaciju i napravi rotaciju objekta
	
	// jednadba 1.5 - os rotacije
	Vertex tanPrije = spline.tangente[iteration];
	Vertex tanPoslije = spline.tangente[iteration + 1];
	...
	cout << kut << endl;
	cout << osRotacije[0] << " " << osRotacije[1] << " " << osRotacije[2] << endl;
	glRotatef(kut, osRotacije[0], osRotacije[1], osRotacije[2]);

	// pomakni se nazad u srediste
	glTranslatef(-object.teziste.x, -object.teziste.y, -object.teziste.z);
...
void myReshape(int w, int h)
{
	...
	glLoadIdentity();
	gluPerspective(45.0, (float)width / height, 0.0, 100.0); // kut pogleda, x/y, prednja i straznja ravnina odsjecanja
